...
def scalarxy(df=None, ycol=None):
...
def xy(df=None, index=None, xcol=None, ycol=None):
...
def nestedxy(df=None, index=None, maincol=None, xcol=None, ycol=None, start_time=None):
    ...
    start_time = start_time if start_time is not None else df['startTime'][index]
    if xcol == 'time':
...
def annotate(ax=None, x=None, y=None, xytext=(0,5), f=None):
    ...
    Args:
      ...
      x: List of x values.
      y: List of y values.
      f: function(i, j) which returns a value, optional.
